...
router.get('/allUser', (req, res, next)=> {
 ...
 //获取所有工作人员
router.get('/allWorker', (req, res, next)=> {
  Worker.find({},(err,doc)=>{
        if(err){
          errTip(res);
        }else{
            ...
            doc.forEach(it=>{
               workerList.push({
                  ...
                  passwordQuestion : it.passwordQuestion, 
                  questionAnswer : it.questionAnswer,
                  OrderNUm:it.workerOrderList.length,
 ...
 //删除工作人员test
// router.get('/deletePerson', (req, res, next)=> {
...
// 	            })
// 	        }else{
// 	          errTip(res,'删除错误,请重试aaaa!');
// 	        }
	                     
// 	    });
// 	});
router.get("/deletePerson", async (req, res, next) => {
  let userId = req.query.userId;
  console.log(userId);
...
router.post('/findPeople', (req, res, next)=> {
  let param = {
   ...
   if(param.grade==0){
        ...
        worker.findOne({userId:param.userId},(err,doc)=>{
              if(err){
                errTip(res);
              }else{
                  if(!doc){
                  ...
                  res.json({
                    ...
                    result:[{
                      ...
                      passwordQuestion : doc.passwordQuestion, 
                      questionAnswer : doc.questionAnswer,
                      OrderNUm:doc.workerOrderList.length,
  ...
  if(param.condition=='订单号'){
      User.findOne({"userOrderList.orderId":param.answers},(err,doc)=>{
        ...
        if(param.condition=='工作人员id'){
            Who = worker; whatOrderList = "workerOrderList";
        }else{
   ...
   if(param.msgWho=='all'){
      let len = param.msgAllList.length;
        ...
        if(param.msgOneGrade==1){
          sendMsg(worker, 'one' ,{userId:param.userId});
        }else{
    ...
    User.findOne({userId:userId},(err,userdoc)=>{
          if(err){
            errTip(res);
              ...
              Admin.findOne({adminId:"13527275970"},(err1,admindoc)=>{
                  if(err1){
                    errTip(res);
                  }else{
                      //是不是第一次申请 
  ...
  User.findOne({userId:userId},(err,userdoc)=>{
        if(err){
          errTip(res);
            ...
            let newworker = new worker(workerData);
            newworker.messages.push(message);
            newworker.save((err3,doc3)=>{
                  if(err3){
