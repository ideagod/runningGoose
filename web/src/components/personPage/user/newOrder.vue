<template>
	<div class="newOrder">
		<person-title>当前位置 :: 我的订单 >> 创建订单 >> <em>发布代取订单</em></person-title>
		<div class="createNewOrder">
			<el-form :model="orderForm" :rules="rules" ref="newOrder" label-position='left' @submit.native.prevent :status-icon='true'
			 label-width="7rem" class="demo-orderForm">
				<el-form-item label="物品类型 : " prop="goodsTpye">
					<el-select v-model="orderForm.goodsTpye" placeholder="请选择物品类型">
						...
						<el-option v-for="goods in goodsTpyeList" :key='goods' :label="goods" :value="goods"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="物品大小 : " prop="goodsSize">
					<el-select v-model="orderForm.goodsSize" placeholder="请选择物品大小">
						<el-option label="请选择" value="请选择" disabled></el-option>
						<el-option v-for="size in goodsSizeList" :key='size' :label="size" :value="size"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="快递公司(可多选) : " prop="company">
					<el-select v-model="orderForm.company" multiple placeholder="请选择快递公司">
						<el-option label="请选择" value="请选择" disabled></el-option>
						<el-option v-for="item in companyList" :key='item' :label="item" :value="item"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="取货码(可多个) : " prop="goodsCode">
					<el-select v-model="orderForm.goodsCode" multiple filterable allow-create default-first-option no-data-text='添加方法: 回车或者选择文本'
					 placeholder="请输入取货码">
					</el-select>
				</el-form-item>
				<el-form-item label="发布价格(1-20元) : " prop="Price">
					<el-input-number v-model="orderForm.Price" :precision="2" :step="1.00" :max="20" :min="1"></el-input-number>
				</el-form-item>
				<el-form-item label="快递数量 : " prop="goodsNumber">
					<el-input-number v-model="orderForm.goodsNumber" :step="1.00" :max="10" :min="1"></el-input-number>
				</el-form-item>
				<el-form-item style='margin-left:0.5rem' label="备注 : " prop="remarks">
					<el-input type="textarea" :rows="3" placeholder="请输入内容..." resize='none' v-model="orderForm.remarks">
					</el-input>
				</el-form-item>
				<el-form-item>
					<el-button type="primary" @click="submitForm('newOrder')">发布</el-button>
					<el-button @click="resetForm('newOrder')">重置</el-button>
					<el-button @click="destroyed()">返回</el-button>
				</el-form-item>
			</el-form>
		</div>
	</div>
			...
			return {
				...
				rules: {
					...
					goodsNumber: [{
						...
						{
							validator: trueGoodsNumber,
							message: '取货码数量和快递数量不匹配 !',
							trigger: 'change'
						}
					]
				}
