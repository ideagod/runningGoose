<template>
    <div class="workerAccount">
        <person-title>当前位置 :: 个人中心 >> <em>我的钱包</em></person-title>
        <div class="workerAccount-cont">
            <div class="workerAccount-body">
                <!-- 用vuex -->
...
export default {
    ...
    methods:{
        getBalance(){
            ...
            axios.get(`/workers/balance`).then(response=>{
        ...
        handleCashWithDraw(){     
            ...
            if(this.balance>0 && trueAlipay && this.errorTip=='' && this.alipayErrorTip=='' && this.alipayAccount!='' && this.WithDrawNum!=''){
                ...
                axios.post('/workers/cashWithDraw',param).then(response=>{
                    let res = response.data;
                    loading.close();
                    if(res.status=='0'){
                        this.getBalance();
