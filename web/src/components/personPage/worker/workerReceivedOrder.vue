<template>
    <div class="workerReceivedOrder">
        <person-title>当前位置 :: 我的订单 >> <em>已接订单</em></person-title>
        <div class="workerReceivedOrder-cont">
            <workerOrderList
...
export default {
    ...
    methods:{
        getReceivedOrderList(){
            ...
            axios.get(`/workers/theReceivedOrder`).then(response=>{
        ...
        handleCompletedOrder(orderData){
            ...
            若客户2天内没有确认则系统自动确认`, 
                ...
                let loading = this.$loading({lock:true,text:'玩命加载中...'});
                axios.post(`/workers/completedOrde`,orderData).then(response=>{
                    let res = response.data;
                    loading.close();
                    if(res.status=='0'){
                        this.$message({
